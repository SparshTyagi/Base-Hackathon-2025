...
app.post("/api/groups", async (req, res) => {
  ...
  const { name, targetAmountEth, durationDays } = req.body as any;
...
app.post("/api/groups/:groupId/join", async (req, res) => {
  ...
  const { bondAmountEth } = req.body as any;
  if (!bondAmountEth) {
...
app.post("/api/groups/:groupId/penalty", async (req, res) => {
  ...
  const { groupId } = req.params as any;
  const { user, amountEth } = req.body as any;
  if (!user || !amountEth) {
...
app.get("/api/groups/:groupId/members/:member", async (req, res) => {
  const { groupId, member } = req.params as any;
...
app.post("/api/violations/webhook", async (req, res) => {
  const { groupId, memberFid, ruleId, violationType, evidence } = req.body as any;
